...
e.innerHTML =
	...
	function (e) {
		e.info = "info", e.warning = "warning", e.error = "error", e.verbose = "verbose", e.success = "success"
	}(n || (n = {}));
class t {
	constructor(e = !0) {
		this.logToConsole = e, window.addEventListener("sai-print-logs", (() => {
	...
	write(e, n, s) {
		if (this.logToConsole) {
			...
			s ? console.log(`%c[${n}] ${e}`, i, s) : console.log(`%c[${n}] ${e}`, i), t.allLogs.push([Date.now(), n,
				e, s
			])
		}
	}
...
class s {
	...
	write(e, n = 3e3) {
		this.isVisible && clearTimeout(this.timer), this.element.innerHTML = e, this.setVisible(!0), this.timer =
			setTimeout((() => {
...
function i(e, n) {
	return e === n || ("zh-CN" === e && "cmn-Hans-CN" === n || ("zh-TW" === e && "cmn-Hant-TW" === n || "zh-HK" === e &&
		"yue-Hant-HK" === n))
}
const a = [
	["English (US)", "en-US"],
...
async function r() {
	...
	return a.forEach((n => {
		e.some((e => i(e.lang, n[1]))) ? o.push(n) : t.warn(
			`${n[0]} not supported. Removed from selector.`)
	})), o
}
class l {
	constructor(e, n) {
		this.selectionCb = e, this.selected = n, this.storageKey = "sai-language", this.setDefaultFromStorage(),
			this.element = document.createElement("div"), this.selector = document.createElement("select"), this
				.element.id = "sai-lang-selector-wrapper", this.selector.id = "sai-lang-selector", r().then((e => {
					if (0 === e.length) {
						this.selector.classList.add("sai-hide");
						const e = document.createElement("div");
						return e.id = "sai-no-voices", e.innerHTML = "No voices", this.element.appendChild(e),
							void (e.onclick = () => {
								let e = "";
					...
					e.forEach((([e, n]) => {
						...
						t.innerText = e, t.value = n, n === this.selected && (t.selected = !0), this
							.selector.appendChild(t)
					})), this.element.appendChild(this.selector), this.selector.onchange = e => {
						const n = e.target;
...
class d {
	constructor(e, n) {
		this.lang = e, this.lastText = "", this.lastRead = Date.now(), this.lastUtter = Date.now(), this
			.lastUtterCharCount = 0, this.lastTimeout = 0, this.lastTimeSinceLastUtter = 0, this.synth = window
				.speechSynthesis, this.queue = [], this.enabled = !0, this.storageKey = "sai-read-aloud", this
					.queueIdle = !0, this.disableButton = document.createElement("div"), this.disableButton.innerHTML =
			...
			this.disableButton.id = "sai-disable-read-aloud", this.disableButton.title = "Toggle read aloud", window
				.speechSynthesis.cancel(), this.disableButton.addEventListener("click", (() => {
					this.enabled ? this.disableReadAloud() : this.enableReadAloud()
				})), this.setReadAloudFromStorage(), t.info(`reInit ${n}, lastTextLength: ${this.lastText.length}`),
			n && this.reset()
	}
	async runQueue() {
		//  && this.queueIdle
		if (t.info(`Queue is idle: ${this.queueIdle}`), this.queue.length > 0) {
			this.queueIdle = !1;
			const e = this.queue.shift();
			await this.readAloud(e), this.queueIdle = !0, this.queue.length > 0 && this.runQueue()
		}
	}
	observerCallback(e) {
		...
		if (s.length > 0 && ("." === i || "?" === i || "!" === i || ":" === i || "。" === i || a)) {
			a && (t.warn(`Long time since last read. Queue length: ${this.queue.length}`), this.queueIdle = !0), t
				.info(`Push to queue: ${s}`);
			s.split(".").filter((e => e.length > 0)).forEach((e => {
				this.queue.push(e)
			})), this.runQueue(), this.lastRead = Date.now(), this.lastText = n
		}
	}
	setLang(e) {
		this.lang = e
	...
	readAloud(e) {
		return new Promise(((n, s) => {
			...
			const h = () => {
				const [e, n] = function (e, n, s, i, a, o) {
					const r = Date.now() - s,
						l = function (e, n, t) {
							let s = 100;
							return "zh-CN" !== e && "zh-TW" !== e && "zh-HK" !== e || (s = 240),
								"zh-TW" === e && (s = 300), "ja-JP" === e && (s = 260), "ko-KR" ===
								e && (s = 240), 7e3 + n * s * (1 / t)
						}(e, a, n);
					if (t.warn(
						`[resumeInfinity] Time since last utter: ${r.toFixed(1)}. Timeout: ${l.toFixed(1)}. Last char count: ${a}`
					), window.navigator.userAgent.search("Mac") > -1 && 0 === r && o > 0) {
						const e = i - o,
							n = e / i * 100;
						t.warn(`Last timeout safety gap: ${e.toFixed(1)}ms. ${n.toFixed(1)}%`), n <
							25 && t.error(`________Safety gap ${n.toFixed(1)}% too low!________`)
					}
					return r > l ? (t.error(`No utter timeout ${l.toFixed(1)} - cancel.`), window
						.speechSynthesis.cancel(), setTimeout((() => {
							window.speechSynthesis.resume()
						}), 50), [0, 0]) : [l, r]
				}(r.lang, d, this.lastUtter, this.lastTimeout, this.lastUtterCharCount, this
					.lastTimeSinceLastUtter);
				this.lastTimeout = e, this.lastTimeSinceLastUtter = n, window.speechSynthesis
					.pause(), window.speechSynthesis.resume(), c = setTimeout(h, 7e3)
			};
			o.addEventListener("error", (e => {
				t.error(`Read aloud error ${e.error}`, e), n(void 0), clearTimeout(c)
			})), o.addEventListener("start", (() => {
				t.info(`Speech has started. Volume: ${o.volume}`), this.lastUtter = Date
					.now(), h()
			...
			})), this.synth.speak(o), this.lastUtterCharCount = a.length
		}))
	}
	enableReadAloud() {
	...
	disableReadAloud() {
		this.queue = [], this.queueIdle = !0, this.synth.cancel(), this.disableButton.classList.add("disabled"),
			this.enabled = !1, this.updateStorage()
	}
	updateStorage() {
	...
	setReadAloudFromStorage() {
		...
		e && (this.enabled = "true" === e, this.enabled ? this.enableReadAloud() : this.disableReadAloud())
	}
	getText() {
		const e = document.querySelectorAll(".content"),
			n = e[e.length - 1] ? [e[e.length - 1]] : [];
		let t = "";
		for (const e of n) "PRE" !== e.nodeName && (t += e.textContent);
		return t = t.replace(/`/g, "").replace(/\*/g, "").replace(/\"/g, "").replace(/\\n/g, "").replace(/\\t/g, "")
			.replace(/\\b/g, "").replace(/（/g, " (").replace(/）/g, ") ").replace(/？/g, "? ").replace(/：/g, ": ")
			.replace(/！/g, "! ").replace(/。/g, ". "), t
	}
	reset() {
		t.warn("RESET read aloud"), this.lastText = this.getText(), this.queue = [], this.lastRead = Date.now()
	}
}
class c {
	constructor(e, n, s) {
		this.lang = e, this.errorMessage = n, this.transcript = "", this.recognition = new webkitSpeechRecognition,
			this.isRecording = !1, this.recognition.continuous = !0, this.recognition.interimResults = !0, this
				.recognition.onstart = () => { }, this.recognition.onresult = e => {
					let n = "";
					for (let t = e.resultIndex; t < e.results.length; ++t) e.results[t].isFinal ? this.isRecording && (
						this.transcript += e.results[t][0].transcript, s(this.transcript)) : n += e.results[t][0]
							.transcript;
	...
	start(e) {
		t.info("Start"), this.endCallback = e, this.recognition.lang = this.lang, this.recognition.start(), this
			.isRecording = !0
	}
	stop() {
		t.info(`Stop: ${this.transcript}`), this.isRecording = !1, this.recognition.stop(), this.endCallback =
			void 0
	}
	reset() {
...
class h {
	constructor(e) {
		this.readAloud = e, this.appIsHidden = "true" === window.localStorage.getItem('"sai-hidden"'), this
			.settingsView = document.createElement("div"), this.settingsView.innerHTML =
			...
			this.settingsView.id = "sai-settings-view", this.settingsView.classList.add("sai-hide")
	}
	setupListeners() {
		...
		s.onclick = () => {
			...
			e && "true" === e ? (window.localStorage.setItem('"sai-hidden"', "false"), this.appIsHidden = !1, i
				.innerText = "Hide Voice Control") : (window.localStorage.setItem('"sai-hidden"', "true"),
					this.appIsHidden = !0, this.readAloud.disableReadAloud(), i.innerText = "Show Voice Control"
			)
		};
		const a = t => {
...
class g {
	constructor(e = !1) {
		...
		const n = document.querySelector("textarea"),
			i = n?.parentElement,
			a = i?.querySelector("#submit-btn");
		if (!n || !i || !a) throw new Error("Missing elements");
		this.chatGptInput = n, this.chatGptInputParent = i, this.chatGptSubmitButton = a, this.saiRoot = document
			.createElement("div"), this.saiRoot.id = "sai-root", this.saiInput = document.createElement("div"), this
				.saiInput.className = n.classList.value, this.saiInputWrapper = document.createElement("div"), this
					.saiInputWrapper.id = "sai-input-wrapper", this.saiInputWrapper.className = this.chatGptInputParent
						.classList.value + " is-idle", this.saiInputWrapper.appendChild(this.saiInput), this.saiRoot
							.appendChild(this.saiInputWrapper), this.chatGptInputParent.after(this.saiRoot), this.saiCancelMsg =
			document.createElement("div"), this.saiCancelMsg.id = "sai-cancel-msg", this.saiCancelMsg.innerHTML =
			"按下Esc可取消", this.saiInputWrapper.appendChild(this.saiCancelMsg), this.saiRecordButton =
			document.createElement("div"), this.saiRecordButton.id = "sai-speech-button", this.saiRecordButton
				.innerHTML =
			...
			this.saiInputWrapper.appendChild(this.saiRecordButton), this.saiButtonWrapper = document.createElement(
				"div"), this.saiButtonWrapper.id = "sai-button-wrapper", this.saiSettingsButton = document
					.createElement("div")
			, this.saiSettingsButton.id = "sai-settings-button", this.saiSettingsButton
				.innerHTML =
			...
			}, this.saiErrorMessage = document.createElement("div"), this.saiErrorMessage.id = "sai-error-message",
			this.saiErrorMessage.innerHTML = "error", this.saiErrorMessage.style.display = "none", this.saiRoot
				.append(this.saiErrorMessage), this.errorMessage = new s(this.saiErrorMessage), this.speech = new c(this
					.language, this.errorMessage, this.speechCallback.bind(this)), this.readAloud = new d(this.language,
						e), this.settings = new h(this.readAloud), this.speechHandlers();
		const o = new l(this.setLanguage.bind(this), p);
		this.saiButtonWrapper.appendChild(o.element), this.saiRoot.appendChild(this.saiButtonWrapper), this
			.saiButtonWrapper.appendChild(this.readAloud.disableButton),
			// this.saiButtonWrapper.appendChild(this.saiSettingsButton),
			this.saiRoot.appendChild(this.settings.settingsView), this.settings
				.setupListeners()
	}
	keyDownHandler(e) {
		const n = e.target;
		if (
			("textarea" === n?.localName ||
				...
				((this.holdSpaceTimer = setTimeout(() => {
					...
					(this.spaceIsDown = !0)),
				"textarea" === n?.localName ||
				("Escape" !== e.code && "KeyQ" !== e.code) || !this.isRecording || (t.info(`Pressed ${e.code}`), this.appToIdle()),
				("Escape" === e.code || "KeyQ" === e.code) && !this.isRecording)
		) {
			t.info(`Pressed ${e.code}. Close settings`);
			document
				.getElementById("sai-settings-view")
				?.classList.add("sai-hide");
		}
		"KeyE" === e.code && this.isRecording && (t.info("Pressed KeyE"), this.chatGptInput.value = this.saiInput
			.innerText, this.appToIdle())
	}
	keyUpHandler(e) {
		this.spaceIsDown && "Space" === e.code && (t.info("Space stop"), this.stopRecording())
	}
	speechHandlers() {
		this.spaceIsDown = !1, this.saiInputWrapper.onclick = () => {
			this.isRecording ? (this.speech.stop(), this.stopRecording()) : (this.startRecording(), this.speech
				.start((() => {
					this.stopRecording()
				})))
		}
	...
	stopRecording() {
		...
		e.length > 0 && (this.chatGptInput.value = e, this.chatGptInput.dispatchEvent(new Event("input", {
			bubbles: !0
		})), this.chatGptSubmitButton.click()), this.appToIdle()
	}
	appToIdle() {
		this.speech.stop(), this.speech.reset(), this.isRecording = !1, this.saiInput.innerText = "", this
			.saiInputWrapper.classList.add("is-idle"), this.spaceIsDown = !1, clearTimeout(this.holdSpaceTimer)
	}
	speechCallback(e) {
...
t.setup();
window.localStorage.getItem("sai-voice-speed") || window.localStorage.setItem("sai-voice-speed", "3");
"true" === window.localStorage.getItem('"sai-hidden"') && document.body.classList.add("sai-hidden")
// , chrome.runtime
// 	.onMessage.addListener(((e, n, t) => ("sai-on-chatgpt-message" === e.key && t({
...
function f(e) {
	...
	m && s && (t.info("Re-init app"), n && n.remove(), u = new g(!0), m = !1, document.addEventListener("keydown", w),
		document.addEventListener("keyup", b)), n && s || (t.warn("App removed"), n && n.remove(), u.readAloud
			.reset(), document.removeEventListener("keydown", w), document.removeEventListener("keyup", b), m = !0),
